...
GOLDEN_LIST = 'golden_features.json'
SUBMISSION_FILE = 'submission034.csv'
N_TRIALS = 50
SEED = 42
...
def load_data():
    ...
    with open(GOLDEN_LIST, 'r') as f:
        features = json.load(f)
        
    print(f"Using {len(features)} Golden Features: {features}")
    ...
    # Handle Categoricals (if any in Golden List)
    for col in features:
        if X_train[col].dtype == 'object' or X_train[col].dtype.name == 'category':
            le = LabelEncoder()
            # Fit on full set to handle unseseen labels (simple handling)
            full_vals = pd.concat([X_train[col], X_test[col]]).astype(str)
            le.fit(full_vals)
...
def objective(trial, X, y, groups):
    # Search Space - ENFORCING REGULARIZATION
    params = {
        ...
        'num_leaves': trial.suggest_int('num_leaves', 15, 63), # Conservative
        'max_depth': trial.suggest_int('max_depth', 3, 10), # Prevent deep trees
        'min_child_samples': trial.suggest_int('min_child_samples', 20, 100), # Prevent overfitting to few samples
...
def train_final_model(best_params, X, y, groups, X_test):
    ...
    for fold, (train_idx, val_idx) in enumerate(gkf.split(X, y, groups)):
        ...
        # Ensure n_estimators is sufficient (Optuna used 1000, we keep it)
        params = best_params.copy()
        params['n_estimators'] = 2000 # Give it room
