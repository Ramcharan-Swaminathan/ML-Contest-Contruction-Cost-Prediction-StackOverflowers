...
def get_vit_embedder():
    ...
    # ViT Base 16-patch
    # Using the standard ImageNet weights
    try:
    ...
    # We want the embedding (representation), not the classification logits.
    # In torchvision ViT, setting heads to Identity gives the [CLS] token output (768 dim).
...
def extract_features(df, image_dir, embedder):
    preprocess = transforms.Compose([
        transforms.Resize(224), # ViT standard resolution
        transforms.ToTensor(),
...
def train_vit():
    ...
    sub.to_csv('submission019.csv', index=False)
    print("Submission saved to submission019.csv")
