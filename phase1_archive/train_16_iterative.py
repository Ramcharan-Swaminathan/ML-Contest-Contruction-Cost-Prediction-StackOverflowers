...
# KEY CHANGE: Using Method 13 (0.2212) as the new teacher
PSEUDO_LABEL_FILE = 'submission013.csv' 

DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

# --- Best Hyperparameters (Unchanged from M13/M09) ---
params = {
...
def train_iterative():
    ...
    # 2. Extract Features (Re-run to be safe and standalone)
    ...
    print(f"Training on combined dataset: {len(X_combined)} rows")
    ...
    sub.to_csv('submission016.csv', index=False)
    print("Submission saved to submission016.csv")
