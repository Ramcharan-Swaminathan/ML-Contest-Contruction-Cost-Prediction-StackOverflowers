        ...
        if s2_path and os.path.exists(s2_path):
            try:
                with rasterio.open(s2_path) as src:
                    ...
                    # TTA 4x (To get robust visual signature)
                    rot_embs = []
...
def add_visual_knn_feature(train_df, test_df, pseudo_df, train_embs, test_embs, k=50):
    ...
    # Target to look up (Using Log Target for smoother averaging)
    y_train = np.log1p(train_df['construction_cost_per_m2_usd'].values)
    
    # 1. Fit NN on Train Embeddings
    nn = NearestNeighbors(n_neighbors=k, metric='cosine', n_jobs=-1)
    nn.fit(train_embs)
    
    # 2. Train Feature (OOF)
    # We find K+1 neighbors, and if the first one is itself (distance ~0), we skip it
    dists, indices = nn.kneighbors(train_embs, n_neighbors=k+1)
    
    train_knn = []
    for i in range(len(train_embs)):
        # self is usually indices[i, 0]
        # We take 1:k+1 to ignore self
    ...
    # 4. Pseudo Feature
    # Pseudo uses Test Embeddings, so same as Test
...
def train_visknn():
    ...
    sub.to_csv('submission021.csv', index=False)
    print("Done. Saved submission021.csv")
