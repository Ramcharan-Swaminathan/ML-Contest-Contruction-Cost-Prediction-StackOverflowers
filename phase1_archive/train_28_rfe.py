...
TOP_K = 100 # Target number of features

# --- Hyperparameters (Slightly relaxed for reduced dim) ---
params = {
    ...
    'num_leaves': 45,       # Slightly simpler
    'feature_fraction': 0.8, # Higher fraction since we have good features now
    'bagging_fraction': 0.90,
    'bagging_freq': 5,
    'min_child_samples': 20, 
    'reg_alpha': 0.1,       # Reduced Reg
    'reg_lambda': 0.1,      # Reduced Reg
    'colsample_bytree': 0.8
}

# --- 1. ResNet Embedding Logic (Copied from M27) ---
        ...
        if s2_path and os.path.exists(s2_path):
            try:
                with rasterio.open(s2_path) as src:
                    ...
                    # No TTA for speed in this step, or keep it? M27 used TTA. Let's keep TTA for consistency.
                    rot_embs = []
...
# --- 2. Explicit Stats & Research Features ---
...
def train_rfe():
    ...
    train_df, test_df, pseudo_df_full = add_visual_knn_feature(train_df, test_df, pseudo_df_full, train_embs, test_embs, k=50)
    
    train_df = pd.concat([train_df, train_emb_df], axis=1)
    test_df = pd.concat([test_df, test_emb_df], axis=1)
    pseudo_df_full = pd.concat([pseudo_df_full, test_emb_df], axis=1)
    ...
    # --- FEATURE SELECTION ---
    print("Running Feature Selection (Coarse Model)...")
    # Train on Full Train (single split usually enough for importance)
    dtrain = lgb.Dataset(X, label=y)
    # Use random param for selection
    sel_params = params.copy()
    sel_params['learning_rate'] = 0.05
    sel_params['num_leaves'] = 31
    model_sel = lgb.train(sel_params, dtrain, num_boost_round=1000)
    
    importance = model_sel.feature_importance(importance_type='gain')
    imp_df = pd.DataFrame({'feature': all_features, 'gain': importance})
    ...
    # --- FINAL TRAINING ---
    print("Starting Final GroupKFold Training on Selected Features...")
    X_sel = train_df[selected_features]
    X_pseudo_sel = pseudo_df_full[selected_features]
    X_test_sel = test_df[selected_features]
    
    gkf = GroupKFold(n_splits=5)
    oof_preds = np.zeros(len(X))
    
    for fold, (tr_idx, val_idx) in enumerate(gkf.split(X_sel, y, groups)):
        X_tr, y_tr = X_sel.iloc[tr_idx], y.iloc[tr_idx]
        X_val, y_val = X_sel.iloc[val_idx], y.iloc[val_idx]
        
        X_tr_fold = pd.concat([X_tr, X_pseudo_sel], axis=0)
        y_tr_fold = pd.concat([y_tr, np.log1p(pseudo_df_full[target_col])], axis=0) # Re-use y_pseudo
    ...
    print(f"M28 GroupKFold RMSLE Score (Top {TOP_K}): {score:.5f}")
    
    # Retrain Full
    print("Retraining on Full Data...")
    X_full = pd.concat([X_sel, X_pseudo_sel], axis=0)
    y_full = pd.concat([y, np.log1p(pseudo_df_full[target_col])], axis=0)
    
    train_data = lgb.Dataset(X_full, label=y_full)
    bst = lgb.train(params, train_data, num_boost_round=1500)
    
    preds_log = bst.predict(X_test_sel)
    ...
    sub.to_csv('submission028.csv', index=False)
    print("Done. Saved submission028.csv")
