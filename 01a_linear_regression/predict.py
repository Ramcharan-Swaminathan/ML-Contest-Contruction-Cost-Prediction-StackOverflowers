...
def process_data(df_train, df_eval, country_name):
    # This function processes both train and eval together to ensure consistency
    # BUT we must learn from Train and apply to Eval separately to avoid leakage (though here it's predict time, so leakage isn't the issue, it's validity)
    # Actually, we MUST use Train stats for Eval.
    
    print(f"\nProcessing {country_name}...")
    
    # 1. Feature Engineering: Interaction & Capital
    # Identify Capital using STRING MATCHING on original data
    # Japan: Tokyo
    ...
    # 3. Target Encoding
    # Learn from Train, Apply to Eval
    ...
    # Apply to Eval (handle unknown categories nicely by filling with global mean, which transform does)
    # Check if 'geolocation_name' exists in eval (it should)
    ...
    # We must drop target from Train X eventually
    ...
    # Drop Capital Features for Philippines
    if country_name == 'Philippines':
        drop_cols_ph = ['is_capital', 'us_cpi_x_is_capital']
        df_train = df_train.drop(columns=drop_cols_ph, errors='ignore')
        df_eval = df_eval.drop(columns=drop_cols_ph, errors='ignore')
...
def predict_lr():
    ...
    # Prepare list to collect predictions
    # We need to keep track of IDs to merge back? 
    # Usually submission format is ID, Prediction. 
    # Let's check if 'data_id' is needed. 
    # The drop_noise step drops 'data_id'. We should save it first.
    ...
    for country in ['Japan', 'Philippines']:
        ...
        # Save indices or IDs for recombination
        eval_indices = df_eval_c.index
        ...
        X_eval = df_eval_proc # Predictions shouldn't have target
        
        # Log Transform Target
        y_train_log = np.log1p(y_train)
        
        # Align columns (just in case)
        # Ensure X_eval has same columns as X_train
        # (Preprocessing should have ensured this, but categorical feature levels might differ?)
        # OneHotEncoder handle_unknown='ignore' handles new categories in Eval.
        
        numeric_features = X_train.select_dtypes(include=[np.number]).columns.tolist()
        categorical_features = X_train.select_dtypes(exclude=[np.number]).columns.tolist()
        
        # Train Model
        print(f"  Training Linear Regression for {country}...")
    ...
    # Sort by ID? Or just save. Usually order doesn't matter if ID is there, but let's check input order.
    # It's nice to output in same order if possible, but concat is fine for Kaggle usually.
